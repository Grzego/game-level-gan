...
learned_agents = os.path.join('experiments', 'run-2')  # os.path.join('learned')
learned_discriminator = os.path.join('experiments', 'run-1')
# learned_discriminator = os.path.join('experiments', 'run-6')
learned_generator = os.path.join('experiments', 'run-1')
...
def train(generator: RaceTrackGenerator, discriminator: RaceWinnerDiscriminator,
    ...
    for agent in agents:
        agent.async_optim(optim.Adam(agent.network.parameters(), lr=5e-6))  # 1e-5 start, 2e-6 later
    ...
    while True:
        ...
        boards = torch.cat((boards,
                            ...
                            predefined_tracks()[:, :num_segments, :],
                            predefined_tracks()[:, :num_segments, :]), dim=0)
